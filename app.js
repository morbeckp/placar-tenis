...
window.addEventListener("beforeinstallprompt", event => {
  ...
  const btn = document.getElementById("installPromptButton");
  if (btn && shouldShowInstallGate()) btn.classList.remove("hidden");
});
...
function getPlatformInfo(){
  ...
  const isEdgA = /EdgA/i.test(ua);
  const isFirefoxAndroid = /Firefox/i.test(ua) && isAndroidDevice;
  const isChromeAndroid = /Chrome/i.test(ua) && isAndroidDevice && !isSamsung && !isEdgA && !isFirefoxAndroid;
  const isSafariIOS = isIOSDevice && /Safari/i.test(ua) && !isCriOS && !isFxiOS && !isEdgiOS;

  let os = "other";
  if (isIOSDevice) os = "ios";
  if (isAndroidDevice) os = "android";

  let browser = "other";
  if (isSafariIOS) browser = "safari_ios";
  else if (isIOSDevice && isCriOS) browser = "chrome_ios";
  else if (isIOSDevice && isFxiOS) browser = "firefox_ios";
  else if (isIOSDevice && isEdgiOS) browser = "edge_ios";
  else if (isChromeAndroid) browser = "chrome_android";
  else if (isSamsung) browser = "samsung_android";
  else if (isFirefoxAndroid) browser = "firefox_android";
  else if (isEdgA) browser = "edge_android";

  return { os, browser };
}

function shouldShowInstallGate(){
  return !isStandaloneMode() && !localStorage.getItem("dismissInstallGuide");
}

function buildInstallSteps(){
  const info = getPlatformInfo();

  if(info.os === "ios" && info.browser === "safari_ios"){
    return {
      lead: "Adicione este placar à Tela de Início para usar como app, com tela cheia e melhor funcionamento.",
      note: "No iOS, a instalação precisa ser feita manualmente pelo Safari.",
      steps: [
        ["Abra o menu de compartilhamento", "Toque no ícone de compartilhar do Safari."],
        ["Adicione à Tela de Início", "Escolha “Adicionar à Tela de Início”."],
        ["Confirme", "Toque em “Adicionar” e abra pelo novo ícone."]
      ]
    };
  }
  ...
  if(info.os === "ios"){
    return {
      lead: "No iPhone, a instalação como app deve ser feita pelo Safari.",
      note: "Você está em outro navegador. Abra este endereço no Safari para adicionar à Tela de Início.",
      steps: [
        ["Copie o endereço", "Copie o link desta página."],
        ["Abra no Safari", "Cole o endereço no Safari."],
        ["Instale", "Toque em Compartilhar > Adicionar à Tela de Início."]
      ]
    };
  }

  if(info.os === "android" && info.browser === "chrome_android"){
    return {
      lead: "Instale este placar como app no Android para abrir em tela cheia.",
      note: deferredInstallPrompt ? "Você pode usar o botão Instalar app abaixo." : "Se o botão nativo não aparecer, use o menu ⋮ do Chrome.",
      steps: [
        ["Use o botão de instalação", "Toque em “Instalar app”, se disponível."],
        ["Ou abra o menu ⋮", "No Chrome, toque nos três pontos no canto superior."],
        ["Instale", "Escolha “Instalar app” ou “Adicionar à tela inicial”."]
      ]
    };
  }
  ...
  if(info.os === "android"){
    return {
      lead: "Adicione este placar à tela inicial para usar como app.",
      note: "O nome do menu pode variar conforme o navegador Android.",
      steps: [
        ["Abra o menu do navegador", "Procure o botão ⋮ ou o menu de opções."],
        ["Escolha instalar", "Toque em “Instalar app” ou “Adicionar à tela inicial”."],
        ["Confirme", "Abra depois pelo ícone criado."]
      ]
    };
  }

  return {
    lead: "Este app pode ser instalado em navegadores compatíveis com PWA.",
    note: "Em computadores, procure o ícone de instalação na barra de endereço ou no menu do navegador.",
    steps: [
      ["Procure instalar", "Veja se há um ícone de instalação na barra de endereço."],
      ["Use o menu", "Também pode estar no menu do navegador."],
      ["Confirme", "Depois abra pelo ícone criado."]
    ]
  };
}
...
function renderInstallGate(){
  ...
  const content = buildInstallSteps();
  const lead = document.getElementById("installLead");
  const list = document.getElementById("installInstructions");
  const note = document.getElementById("installBrowserNote");
  const btn = document.getElementById("installPromptButton");

  if(lead) lead.innerText = content.lead;
  if(note) note.innerText = content.note || "";
  ...
  if(list){
    ...
    content.steps.forEach((step, index) => {
      ...
      item.innerHTML = `<div class="install-step-number">${index + 1}</div><div><strong>${step[0]}</strong><span>${step[1]}</span></div>`;
  ...
  const info = getPlatformInfo();
  if(btn){
    if(info.os === "android" && deferredInstallPrompt){
...
function dismissInstallGuide(){
  localStorage.setItem("dismissInstallGuide", "1");
  const gate = document.getElementById("installGate");
